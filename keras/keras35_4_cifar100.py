...
# 2. 모델 구성 
model = Sequential()
model.add(Conv2D(filters=1024, kernel_size=(3, 3), strides=2, padding='same', input_shape=(32, 32, 3), activation='relu')) # (27, 27, 128)
model.add(MaxPooling2D())
model.add(Conv2D(filters=512, kernel_size=(2, 2)))                             # (26, 26, 64)
model.add(MaxPooling2D())
model.add(Conv2D(filters=256, kernel_size=(2, 2)))                             # (25, 25, 64)
model.add(Flatten())
...
model.fit(x_train, y_train, epochs=100, batch_size=256, verbose=1, validation_split=0.2, callbacks=[es, mcp])
